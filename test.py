...
def parse_args():
    ...
    parser.add_argument('--name', default='jiu0Monkey_Unettrans_woDS',
                        help='model name')
...
def main():
    ...
    model = unettrans.__dict__[args.arch](args) #调用模型unettrans
    ...
    # Data loading code
    val_img_paths = glob(r'/home/sophia/桌面/czs/2d/testImage/*')
    val_mask_paths = glob(r'/home/sophia/桌面/czs/2d/testMask/*')
    ...
    if val_args.mode == "Calculate":
        ...
        print("=============")
        with open('models/%s/testindex.txt' %args.name, 'w') as f:
            f.write('WT Dice: %.4f' % np.mean(wt_dices))
            f.write('\n')
            f.write('TC Dice: %.4f' % np.mean(tc_dices))
            f.write('\n')            
            f.write('ET Dice: %.4f' % np.mean(et_dices))
            f.write('\n')
            f.write('WT PPV: %.4f' % np.mean(wt_ppvs))
            f.write('\n')
            f.write('TC PPV: %.4f' % np.mean(tc_ppvs))
            f.write('\n')
            f.write('ET PPV: %.4f' % np.mean(et_ppvs))
            f.write('\n')
            f.write('WT sensitivity: %.4f' % np.mean(wt_sensitivities))
            f.write('\n')
            f.write('TC sensitivity: %.4f' % np.mean(tc_sensitivities))
            f.write('\n')
            f.write('ET sensitivity: %.4f' % np.mean(et_sensitivities))
            f.write('\n')
            f.write('WT Hausdorff: %.4f' % np.mean(wt_Hausdorf))
            f.write('\n')
            f.write('TC Hausdorff: %.4f' % np.mean(tc_Hausdorf))
            f.write('\n')
            f.write('ET Hausdorff: %.4f' % np.mean(et_Hausdorf))
            f.write('\n')
